...
- Regressão observada nesse teste: os quatro cards recentes da Home mostraram o ícone no lugar das fotos. A correção mantém a atribuição posterior apenas para a grade da Coleção e entrega as capas reais diretamente aos cards da Home, que reutilizam `card()`. Validar novamente a Home no aparelho antes do checkpoint posterior.
- Correção complementar da Home: `renderHome()` usava `.map(card)`, que passa `(figura, índice)`; depois da nova assinatura de `card(f, withoutCovers, coverIndex)`, os índices 1–3 ativavam acidentalmente o modo sem capas. A chamada agora usa `.map(f => card(f))`, sem repassar o índice. Teste estático confirmou as quatro capas; validação visual no aparelho ainda pendente.
